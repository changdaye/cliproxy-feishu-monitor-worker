...
- **飞书通知**：北京时间 `09:00 / 12:00 / 19:00` 推送汇总，并包含 Token 用量、心跳、异常提醒
- **手动控制**：支持手动触发一轮巡检
...
- 定时调度（Cloudflare Cron：北京时间 `09:00 / 12:00 / 19:00`）
- Queue 分片执行
- D1 批次 / 分片 / 报表存储
- CLIProxyAPI 配额抓取
- Token 用量汇总
...
当前默认定时点为北京时间 `09:00 / 12:00 / 19:00`（Cloudflare Cron 对应 UTC `01:00 / 04:00 / 11:00`）。
